...
class TestGrade(unittest.TestCase):
    ...
    def test_name_mismatch(self):
        cases = [
            ...
            (
                ...
                # matched=1, max(2,2)=2, name_score=0.5 → tool=0.5*0.5=0.25 → 0.9*0.25+0.1=0.33
                0.33,
            ),
            (
                ...
                # matched=1, max(1,2)=2, name_score=0.5 → tool=0.25 → 0.9*0.25+0.1=0.33
                0.33,
            ),
            (
    ...
    def test_argument_scoring(self):
        cases = [
            (
                ...
                # name_score=1.0, arg_score=0.0 → tool=0.5 → 0.9*0.5+0.1=0.55
                0.55,
            ),
            (
                ...
                # 1 of 2 leaves match → avg=0.5 → tool=0.5+0.5*0.5=0.75 → 0.9*0.75+0.1=0.78
                0.78,
            ),
            (
                ...
                {"output_tools": [_call("f", {"a": 1})]},
                {"reference_tool_calls": [_ref("f", {"a": 1, "b": 2})]},
                # a matches (1,1), b missing (0,1) → 1/2=0.5 → 0.78
                0.78,
            ),
            (
    ...
    def test_nested_arguments(self):
        cases = [
            ...
            (
                ...
                {"reference_tool_calls": [_ref("f", {"a": {"x": 1, "y": 2}})]},
                # 1 of 2 leaves → avg=0.5 → tool=0.75 → 0.9*0.75+0.1=0.78
                0.78,
            ),
            (
            ...
            (
                ...
                {"reference_tool_calls": [_ref("f", {"ids": [1, 2, 3]})]},
                # 2 of 3 list items match → avg=2/3 → tool=0.83 → 0.9*0.83+0.1=0.85
                0.85,
            ),
            (
                ...
                {"reference_tool_calls": [_ref("f", {"ids": [1, 2, 3]})]},
                # 1 of 3 → avg=1/3 → tool=0.67 → 0.9*0.67+0.1=0.7
                0.7,
            ),
        ]
        self._run_cases(cases)
    ...
    def test_include_tools_missing_actual(self):
        ...
        self.assertAlmostEqual(grade_with_config(sample, item, config), 0.1)
...
class TestGradeText(unittest.TestCase):
    ...
    def test_empty_reference(self):
        self.assertAlmostEqual(grade_text("some output", ""), 1.0)
...
class TestCombinedGrade(unittest.TestCase):
    ...
    def test_perfect_tools_no_text(self):
        ...
        score = _grade_all(sample, item)
        # tool_score=1.0, no expected text → text_score=1.0 → 1.0
        self.assertAlmostEqual(score, 1.0)
